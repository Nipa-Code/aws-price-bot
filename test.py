...
def read_user_args(args: str) -> argparse.Namespace:
    ...
    # this section is useless for now, since it's not used anywhere
    parser.add_argument(
        "-e",
        "--allow-only-empty",
        action="store_true",
...
def add_alias(name: str = "setup-project", command: str = None) -> None:
    """
...
def del_even_readonly(action, name, exc) -> None:
    """
...
def create_template(
    ...
    """
    if os.listdir():  # check if path is empty. NOTE: add "not" if it is not there
        logger.info(
